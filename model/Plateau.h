...
class  Plateau {
    ...
    int nbPiecesB;
    int nbPiecesN;
    //int board [10] [10];  // repère , la case en haut à gauche à pour pposition (0,0)
...
public:
    ...
    /* pour les méthodes de modication de board après un déplacement:
    ...
    */
    ...
    void listeCheminsPrise(Pion p, std::vector<Case> chemin, std::vector<std::vector<Case>> &listeChemins,
                           int &max); // Construit la liste de chemins avec prises possible pour le pion
    ...
    */
    bool
    verifDeplacement(Pion pion, Case cible, int nbDepla, std::map<Case, std::vector<std::vector<Case>>> cheminsPieces);

    bool
    verifDeplacement(Dame dame, Case cible, int nbDepla, std::map<Case, std::vector<std::vector<Case>>> cheminsPieces);
